...
@section('settings-title')
    {{ __('global.edit') }} {{ __('cruds.room.title_singular') }}
@endsection
...
@section('settings-breadcrumbs')
    {{ Breadcrumbs::render('settings.rooms.edit', $room) }}
@endsection
    ...
    <div class="card mb-6 mb-xl-9">
        <div class="card-header border-0">
            <div class="card-title">
                <h2>{{ __('global.edit') }} {{ __('cruds.room.title') }}</h2>
            </div>
        </div>
        <div class="card-body pt-0 pb-5">
            <form method="POST" action="{{ route('settings.rooms.update', $room->room_id) }}" enctype="multipart/form-data">
                @method('PUT')
                @csrf
                ...
                <div class="fv-row mb-7">
                    ...
                    <input type="text" class="form-control {{ $errors->has('room_name') ? 'is-invalid' : '' }}"
                        placeholder="" name="room_name" value="{{ old('room_name', $room->room_name) }}" required />
