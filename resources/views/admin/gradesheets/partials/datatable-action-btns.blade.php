<div class="d-flex justify-content-end flex-shrink-0">
    ...
    @isset($editGate)
        @can($editGate)
            <a href="{{ isset($primaryKey) ? route('admin.gradesheet.edit', $row[$primaryKey]) : route('admin.gradesheet.edit', $row->id) }}"
                class="btn btn-icon btn-light-warning btn-active-warning btn-sm me-1" data-bs-toggle="tooltip" data-bs-placement="right" data-bs-custom-class="tooltip-dark"
    ...
    @can('gradesheet_generate')
        <a href="{{ isset($primaryKey) ? route('admin.gradesheet.generate.pdf', $row[$primaryKey]) : route('admin.gradesheet.generate.pdf', $row->id) }}"
            class="btn btn-sm btn-icon btn-light btn-active-secondary me-1" target="_blank" data-bs-toggle="tooltip" data-bs-placement="right" data-bs-custom-class="tooltip-dark"
    ...
    @isset($deleteGate)
        @can($deleteGate)
            ...
            <form
                ...
                method="POST" id="{{ $formId }}-{{ $resource }}-destroy" data-bs-toggle="tooltip" data-bs-placement="right" data-bs-custom-class="tooltip-dark"
